...
class API extends REST 
{
	...
	{ 
		...
		$sql = mysql_query("SELECT DATE_FORMAT(Temp_releve.Date, \"%d/%m/%Y à %H:%i\") as Date,Temp_releve.Temp_Ext,Temp_releve.Temperature, Temp_releve.Humidite, Temp_releve.Tp_Consigne, CONCAT(Radiateurs.Piece, ' (',Temp_releve.Radiateur, ')') as Piece, Temp_releve.Allume FROM Temp_releve, Radiateurs where Temp_releve.Radiateur = Radiateurs.Numero AND Radiateurs.Numero < 9 ORDER BY Temp_releve.Date desc", $this->db);
		if(mysql_num_rows($sql) > 0)
		{
			$result = array();
			$line = 0;
	...
	private function check_rad() //Renvoie les dernières mesures par radiateur
	{ 
		// Cross validation if the request method is GET else it will return "Not Acceptable" status
		if($this->get_request_method() != "GET")
		{
		...
		$sql = mysql_query("SELECT distinct Numero FROM Radiateurs where Numero<8", $this->db);
		if(mysql_num_rows($sql) > 0)
		{
			$result = array();
			while($radia = mysql_fetch_array($sql,MYSQL_ASSOC))
			{
				if(mysql_num_rows($sql1) > 0)
				{
					$sql1 = mysql_query("SELECT max(date),Temperature FROM Temp_releve WHERE Radiateur= ".$radia." order by date desc", $this->db);
					$result[] = $rlt;
			}
			// If success everything is good send header as "OK" and return list of users in JSON format
			$this->response($this->json($result), 200);
